...
import time
from .. import items, games, engine, defaults
...
class WarehouseGame (games.GameBase_multiagent):
    ...
    def __init__(
        ...
        self.reward_each_action = -0.05
        self.reward_illegal_action = -0.1
    ...
    def _create_game(self, goal_area:bool = False):
        ...
        # Generate all possible coordinates with step 2 within the given boundaries
        all_possible_coordinates = [(x, y) for x in range(2, self.shape[0]-3) for y in range(2, self.shape[1]-3)]

        #Randomly select the desired number of coordinates
        goals_coordinates = random.sample(all_possible_coordinates, self.num_objects)
        for (x,y) in goals_coordinates: # TODO if goal_area == False, self.num_objects is not used
            self.goals.append(items.GoalItem((x, y)))
          

        #fixed agents and goals placements
        ...
        # while len(goals_coordinates) < self.num_objects:
        #     for x in range(x_start, self.shape[0] - 3, step):
        #         for y in range(y_start, self.shape[1] - 3, step):
        #             if len(goals_coordinates) < self.num_objects:
        #                 goals_coordinates.append((x, y))
        #             else:
        #                 break

        # for (x,y) in goals_coordinates:
        #     self.goals.append(items.GoalItem((x, y)))

        # agents_coordinates = []
        # x_start = self.shape[0]-4
        # y_start = self.shape[0]-4
        # step = 2
        # possible_goal_coordinates = [(x,y) for x in range(x_start, 1, -step) for y in range(y_start, 1, -step) ]
        # possible_goal_coordinates = [(x,y) for (x,y) in possible_goal_coordinates if (x,y) not in agents_coordinates]
        # for i, agent_id in enumerate(self.agent_ids):
        #     agents_coordinates.append(possible_goal_coordinates[i])
        #     agent = items.PickAgent(possible_goal_coordinates[i], kind = agent_id)
        ...
        # self.goals, goals_coordinates = self.create_goal_cells(num_cells = self.agent_ids, grid_shape = self.shape)
        ...
        # object_cells = [object_cell for object_cell in object_cells 
        #                 if (object_cell[0], object_cell[1]) not in goals_coordinates and (object_cell[0], object_cell[1]) not in agents_coordinates]

        #for random agents placements setting
        object_cells = [object_cell for object_cell in object_cells 
                            if (object_cell[0], object_cell[1]) not in goals_coordinates]
        ...
        #fixed agent placements
        # self.engine._add(list(self.agent_dict.values()))

    #random agents placement
    ...
    def is_adjacent_to_impassable(self, direction, item):
        """
    ...
    def is_adjacent_to(self, direction, item, other_item):
        """
