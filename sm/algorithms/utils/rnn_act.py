...
def discrete_autoregreesive_act(decoder, obs_rep, obs, batch_size, n_agent, action_dim, tpdv,
                                available_actions=None, deterministic=False, state=None):
    ...
    for i in range(n_agent):
        if state is None:
            logit_, state = decoder(shifted_action, obs_rep, None)
            logit = logit_[:, i, :]
        else:
            logit_, state = decoder(shifted_action, obs_rep, state)
            logit = logit_ [:, i, :]
...
def continuous_autoregreesive_act(decoder, obs_rep, obs, batch_size, n_agent, action_dim, tpdv,
                                  deterministic=False, state=None):
    ...
    for i in range(n_agent):
        if state is None:
            ...
            act_mean_, state =  decoder(shifted_action, obs_rep, state)
            act_mean = act_mean_[:, i, :]
...
def discrete_parallel_act(decoder, obs_rep, obs, action, batch_size, n_agent, action_dim, tpdv,
    ...
    # None is no state passed
    logit, _ = decoder(shifted_action, obs_rep, None)
    if available_actions is not None:
...
def continuous_parallel_act(decoder, obs_rep, obs, action, batch_size, n_agent, action_dim, tpdv):
    ...
    act_mean, _ = decoder(shifted_action, obs_rep, None)
    action_std = torch.sigmoid(decoder.log_std) * 0.5
