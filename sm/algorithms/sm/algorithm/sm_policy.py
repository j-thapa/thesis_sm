import torch
import numpy as np
from sm.utils.util import update_linear_schedule
from sm.utils.util import get_shape_from_obs_space, get_shape_from_act_space
from sm.algorithms.utils.util import check
from sm.algorithms.sm.algorithm.ma_transformer import MultiAgentTransformer
import time


class SMPolicy:
    """
     SM Policy  class. Wraps actor and critic networks to compute actions and value function predictions.
    ...
    def __init__(self, args, obs_space, cent_obs_space, act_space, num_agents, device=torch.device("cpu")):
        ...
        if self.algorithm_name in ["mat", "mat_dec"]:
            from sm.algorithms.sm.algorithm.ma_transformer import MultiAgentTransformer as SM
        elif self.algorithm_name == "mat_gru":
            from sm.algorithms.sm.algorithm.mat_gru import MultiAgentGRU as SM
        elif self.algorithm_name == "mat_decoder":
            from sm.algorithms.sm.algorithm.mat_decoder import MultiAgentDecoder as SM
        elif self.algorithm_name == "mat_encoder":
            from sm.algorithms.sm.algorithm.mat_encoder import MultiAgentEncoder as SM
        elif self.algorithm_name == "marwkv_v6":
            from sm.algorithms.sm.algorithm.marwkv_v6 import MultiAgentRWKV as SM
        elif self.algorithm_name == "marwkv_v5":
        ...
        self.seq_model = SM(self.share_obs_dim, self.obs_dim, self.act_dim, num_agents,
        ...
        # model_dir = "/home/PROFACTOR.LOCAL/jthapa/rware/sm/scripts/results/WarehouseEnv/mat/shape_12_ag_1_obj_2/run5/models/seq_model_1480.pt"
    ...
    def get_actions(self, cent_obs, obs, rnn_states_actor, rnn_states_critic, masks, available_actions=None,
        ...
        actions, action_log_probs, values = self.seq_model.get_actions(cent_obs,
                                                                         obs,
                                                                         available_actions,
                                                                         deterministic)
    ...
    def get_values(self, cent_obs, obs, rnn_states_critic, masks, available_actions=None):
        ...
        values = self.seq_model.get_values(cent_obs, obs, available_actions)
    ...
    def evaluate_actions(self, cent_obs, obs, rnn_states_actor, rnn_states_critic, actions, masks,
        ...
        action_log_probs, values, entropy = self.seq_model(cent_obs, obs, actions, available_actions)
    ...
    #     self.seq_model.load_state_diseq_model_state_dict)
    #     # self.seq_model.reset_std()
    ...
    def restore(self, model_dir):
        ...
        for k, v in seq_model_state_dict.items():
            if k in model_state_dict and model_state_dict[k].size() == v.size():
                ...
                skipped_keys.append(k)  # Log skipped key
        
        # Update the current model's state dict with the matching parameters
        model_state_dict.update(new_state_dict)
        
        # Load the updated state dict into the model
        self.seq_model.load_state_dict(model_state_dict, strict=False)
        
        # Print the loaded and skipped parameters for transparency
        print(f"Policy loaded from {model_dir}.")
        if loaded_keys:
