...
class Encoder(nn.Module):
    ...
    def forward(self, state, obs):
        ...
        if self.encode_state:
            ...
            if len(obs.shape) > 3 :
                batch_size, sequence_length, height, width, channels = obs.shape
                # Reshape to (batch_size * sequence_length, channels, height, width) for CNN
                obs_reshape = obs.reshape(batch_size * sequence_length, height, width, channels).permute(0, 3, 1, 2)
                obs_embeddings = self.obs_encoder(obs_reshape)
                x = obs_embeddings.view(batch_size, sequence_length, -1)
            else:
